...
maxsignals = 5000;
% perm = randperm(length(unused_sigs));
perm = randperm(numel(unused_sigs)); % 
perm = perm(1:min(maxsignals,end));
err = sum((X(:,unused_sigs(perm)) - D*Gamma(:,unused_sigs(perm))).^2); % this looks good
...
for j = 1:dictsize
  ...
  if ( (max(Gj.^2) > muthresh^2) || (~replaced_atoms(j) && usecount(j)<use_thresh) )
    [y,i] = max(err);
    try % debug, sometimes this cannot work
        d = X(:,unused_sigs(perm(i))); % for unused_sigs, it is the size of data dimension, an array
        D(:,j) = d/norm(d);
        unused_sigs = unused_sigs([1:perm(i)-1,perm(i)+1:end]); % unused_sigs changed here
